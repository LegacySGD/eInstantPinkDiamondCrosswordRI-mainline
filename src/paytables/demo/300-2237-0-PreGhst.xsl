			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNames)
					{
						...
						var regPatt = /\|.*/g;
						var notGrid = regPatt.exec(scenario).toString().slice(1);
						var crosswordLetters = getRegularLetters(notGrid);
						var crosswordBonus = getBonusLetters(notGrid);
						var crosswordAllLetters = crosswordLetters + crosswordBonus;
						var convertedPrizeValues = (prizeValues.substring(1)).split('|');
						var crosswordSize = [11,11];
						//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////			
						...
						for(var word = 0; word < CrosswordWords.length; ++word)
								{
							if(checkMatch(crosswordAllLetters, CrosswordWords[word]))
									{
						...
						r.push('</tr>');
						r.push('<tr><td class="tablehead" colspan="' + crosswordLetters.length + '">');
 						r.push(getTranslationByName("bonusLetters", translations));
 						r.push('</td></tr>');
 						r.push('<tr>');
 						for(var letter = 0; letter < crosswordBonus.length; ++letter)
								{
								r.push('<td class="tablebody">');
 							r.push(crosswordBonus[letter]);
								r.push('</td>');
								}
								r.push('</tr>');
						r.push('</table>');
					...
					function getRegularLetters(allLetters)
					{
						var result = '';
